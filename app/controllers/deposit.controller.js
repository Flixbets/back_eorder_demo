  ...
  deposit
    ...
    .then(async (user) => {
      ...
      let peopledata = await people.findOne({
        attributes: ["firstname", "lastname", "username", "credit"],
...
exports.createWithdraw = async (req, res) => {
  ...
  deposit
    ...
    .then(async (user) => {
      ...
      let peopledata = await people.findOne({
        attributes: ["firstname", "lastname", "username", "credit", "creditwithdraw","pincode"],
        where: { id: req.body.peopleId },
      });
      peopledata = JSON.stringify(peopledata);
      peopledata = JSON.parse(peopledata);
      ...
      return await deposit
        .create(data_deposit)
        .then((data) => {
          people.increment("creditwithdraw", {
            by: -amount,
            where: { id: req.body.peopleId },
          });
          res.status(200).send({ status: true });
...
exports.updateWithdraw = async (req, res) => {
  ...
  if (status === 1) {
    ...
    deposit
      .update(
        { status: status, userId: userId, preamount: peopledata.creditwithdraw, net: Number(peopledata.creditwithdraw) + Number(amount) },
        {
          where: { id: id },
        }
      )
